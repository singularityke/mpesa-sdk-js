...
The SDK automatically handles C2B callbacks through the catch-all handler:
...
In the sandbox environment, you can simulate C2B payments by having a test customer send money to your test shortcode:

1. Use a test M-Pesa phone number (provided by Safaricom)
2. Navigate to M-Pesa menu on test phone
3. Select "Lipa na M-Pesa" → "Pay Bill"
4. Enter your test shortcode (e.g., 600000)
5. Enter account number (bill reference)
6. Enter amount
7. Enter M-Pesa PIN

Your validation and confirmation callbacks will be triggered automatically.
...
**Note**: M-Pesa masks customer phone numbers in C2B callbacks. You'll only receive:

- First name
- Partial phone number (first few digits visible)
- Full transaction details

This is by design for customer privacy. If you need to link payments to specific customers, use the bill reference number (account number) that customers enter when paying.
